...
from domainbed.lib.misc import random_pairs_of_minibatches, ParamDict, one_hot, temporal_smooth_loss
...
class AbstractDANN(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
        ...
        super(AbstractDANN, self).__init__(input_shape, num_classes, num_domains,
                                           hparams)
        print("input_shape:", input_shape, "num_classes:", num_classes, "num_domains:", num_domains, "hparams:",
              hparams)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        # print("disc_labels:",disc_labels.shape)
        # count_0 = 0
        # count_1 = 0
        # count_2 = 0
        # count_3 = 0
        # count_4 = 0
        # for domain_label in disc_labels:
        #     if domain_label==0:
        #         count_0+=1
        #     elif domain_label == 1:
        #         count_1 += 1
        ...
        #     elif domain_label == 4:
        #         count_4 += 1
        # print("count_0:", count_0,"count_1:", count_1,"count_2:", count_2,"count_3:", count_3,"count_4:", count_4)
...
class Plain(Algorithm):
    ...
    def __init__(self, input_shape, num_classes, num_domains,
                 hparams):
        super(Plain, self).__init__(input_shape, num_classes, num_domains, hparams)
        print("input_shape:", input_shape, "num_classes:", num_classes, "num_domains:", num_domains, "hparams:",
              hparams)
        self.register_buffer('update_count', torch.tensor([0]))
        # self.hparams = hparams
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        domain_size = all_z.shape[0] // self.num_domains
        domain_certer = []
        for i in range(self.num_domains):
            z_temp = all_z[i * domain_size: (i + 1) * domain_size, :]
            center_temp = torch.mean(z_temp, dim=0)
            # print(center_temp.shape)
            domain_certer.append(center_temp)
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        for m in range(len(minibatches)):
            if self.hparams['dm_idx']:
                ...
                z = self.featurizer(x)
                z = torch.cat([z, d.unsqueeze(1)], dim=-1)
                losses[m] = F.cross_entropy(self.classifier(z), y)
            else:
                x, y = minibatches[m]
...
class AbstractMMD(ERM):
    ...
    def update(self, minibatches, unlabeled=None):
        ...
        if self.hparams['dm_idx']:
            all_x = [x for x, y, _ in minibatches]
            d = [d for _, _, d in minibatches]
            features = [self.featurizer(x) for x in all_x]
            features = [torch.cat([features[i], d[i].unsqueeze(1)], dim=-1) for i in range(nmb)]
...
class MTL(Algorithm):
    ...
    def predict_embs(self, x, env=None):
        features = self.featurizer(x)
        embedding = self.update_embeddings_(features, env).normal_()
        preds = self.classifier(torch.cat((features, embedding), 1))
...
class DDA(Algorithm):
    """
    Directional Domain Augmentations
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(DDA, self).__init__(input_shape, num_classes, num_domains,
                                  hparams)
        ...
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        if self.featurizer.n_outputs > 512:
            self.featurizer.network.fc = nn.Sequential(nn.Linear(self.featurizer.n_outputs, 512),
                                                       nn.ReLU())
            self.featurizer.n_outputs = 512

        if self.featurizer.n_outputs >= 32:
            self.classifier = MetaSequential(
                MetaLinear(self.featurizer.n_outputs, self.featurizer.n_outputs // 2),
                nn.ReLU(),
                MetaLinear(self.featurizer.n_outputs // 2, self.featurizer.n_outputs // 4),
                nn.ReLU(),
                MetaLinear(self.featurizer.n_outputs // 4, num_classes))
            self.d = networks.DomainAdaptor(self.featurizer.n_outputs, hparams, 8, networks.MLP(self.featurizer.n_outputs, self.featurizer.n_outputs, hparams))
        elif self.featurizer.n_outputs == 2:
            self.classifier = MetaSequential(MetaLinear(self.featurizer.n_outputs, num_classes))
            self.d = networks.DomainAdaptor(self.featurizer.n_outputs, hparams, 1, networks.MLP(self.featurizer.n_outputs, self.featurizer.n_outputs, hparams))
        else:
            self.classifier = MetaSequential(
                MetaLinear(self.featurizer.n_outputs, 4),
                nn.ReLU(),
                MetaLinear(4, num_classes))
            self.d = networks.DomainAdaptor(self.featurizer.n_outputs, hparams, 1, networks.MLP(self.featurizer.n_outputs, self.featurizer.n_outputs, hparams)) 
    ...
    def update(self, minibatches, unlabeled=None, return_z=False):
        ...
        z = [self.featurizer(x) for x, _, _ in minibatches]  
        ...
        for i in range(len(minibatches)):
            ...
            c_params = None
            if i != 0:
                c_params, inner_loss = self.inner_loop(z[i-1], z_tld[i-1], int(d[i][0]-1), y[i-1])
                objective += F.cross_entropy(self.classifier(z[i], params=c_params), y[i])
                self.classifier_params[int(d[i][0])] = c_params 
            elif i == 0:
                inner_c_loss = F.cross_entropy(self.classifier(z[i], params=c_params), y[i])  
                c_params = self.gradient_update_parameters(self.classifier, inner_c_loss,
                                                           step_size=self.hparams["alpha"],
                                                           params=c_params)
                objective += F.cross_entropy(self.classifier(z[i], params=c_params), y[i])
                self.classifier_params[int(d[i][0])] = c_params 

            # generate target domain parameters
            if i == T - 1:
                c_params, inner_loss = self.inner_loop(z[i], z_tld[i], int(d[i][0]), y[i], test=True)
    ...
    def dm_adapt(self, z):
        ...
        return self.d(z_cat, z_kv, all_masks).view(T, n, -1)

    def inner_loop(self, z, z_tld, d, y, test=False):
        """

        :param z_tld: \tilde{z} pseudo samples
               z:
               d: domain index, use the classifier from the domain of the real samples z
        :return:
        """
        eps = 1e-6
        c_params = None
        ...
        logit_r = F.softmax(
            self.classifier(z, params=self.classifier_params[d]) / self.hparams["tau_temp"]) + eps
        ...
        for _ in range(iteration):
            logit_p = F.softmax(self.classifier(z_tld, params=c_params) / self.hparams["tau_temp"]) + eps
            inner_loss = self.hparams["lambda"] * F.cross_entropy(self.classifier(z_tld, params=c_params), y) \
                           + (1 - self.hparams["lambda"]) * (self.hparams["tau_temp"] ** 2) * F.kl_div(logit_p.log(),
                                                                                                 logit_r.detach())
            c_params = self.gradient_update_parameters(self.classifier, inner_loss,
                                                       step_size=self.hparams["alpha"],
                                                       params=c_params)
    ...
    def predict(self, x, d):
        ...
        d = torch.unique(d)
        z = self.featurizer(x)
        preds = self.classifier(z, params=self.classifier_params[int(d if d<self.T else -1)])
        return preds
